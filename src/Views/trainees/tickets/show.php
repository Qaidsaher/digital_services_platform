...
<div class="container mx-auto py-2">
  <div class="max-w-7xl mx-auto bg-white shadow-lg rounded-lg p-6">
          ...
          <?php foreach($comments as $comment): ?>
            <div class="bg-gray-100 border border-gray-200 rounded-md p-4 shadow-sm">
              <p class="text-gray-700 mb-2"> 
                ...
                <?= nl2br(htmlspecialchars($comment->text ?? 'No comment text available')) ?>
              </p>
              <p class="text-sm text-gray-500">
                <i class="fas fa-calendar text-gray-500"></i> 
                By <strong><?= htmlspecialchars($comment->creator_type) ?></strong> on <?= htmlspecialchars($comment->date) ?>
              </p>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <div class="mt-8">
      <h2 class="text-2xl font-semibold text-gray-900 mb-4 flex items-center">
        <i class="fas fa-comment-dots text-blue-600 mr-3"></i> Add a Comment
      </h2>
      <form action="<?= route('trainee.comments.store') ?>" method="POST" class="bg-white p-6 shadow-md rounded-md border border-gray-200">
    ...
    <div class="text-right mt-6">
      <a href="<?= route('trainee.tickets.index') ?>" class="px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 rounded-md shadow-md font-medium">
...
<!-- JavaScript to Hide Success/Error Messages After 5 Seconds -->
<script>
  setTimeout(() => {
