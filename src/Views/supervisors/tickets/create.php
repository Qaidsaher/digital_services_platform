...
<div class="container mx-auto py-8">
  <div class="max-w-7xl mx-auto bg-white shadow-xl rounded-lg p-8">
    ...
    <form action="<?= route('supervisor.tickets.create') ?>" method="POST" class="space-y-6">
      <div>
      ...
      <div>
        ...
        $statusColors = [
          ...
          "In Progress" => "bg-yellow-600 hover:bg-yellow-700 text-white border-yellow-600",
        ...
        <div class="flex space-x-4 mt-2">
          <?php foreach ($statuses as $status): ?>
            <label
              ...
              class="status-label cursor-pointer px-2 py-1 rounded-sm border transition-all duration-300 <?= $defaultStatus === $status ? $statusColors[$status] : "bg-white text-gray-600 border-gray-300" ?>">
              <input type="radio" name="status" value="<?= $status ?>" <?= $defaultStatus === $status ? "checked" : "" ?> class="hidden">
              <?= $status ?>
            </label>
          <?php endforeach; ?>
        </div>
